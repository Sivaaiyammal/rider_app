    ...
    <View style={ss.slot}>
      {loading ? (
        <>
          ...
          <Text style={ss.slotLabel}>{label}</Text>
        </>
      ) : image ? (
        <>
          ...
          <View style={ss.slotFooter}>
            ...
            <TouchableOpacity style={ss.reBtn} onPress={() => handle('camera')} activeOpacity={0.8}>
              ...
              <Text style={ss.reTxt}>Retake</Text>
            </TouchableOpacity>
          </View>
        </>
      ) : (
        <>
          ...
          <View style={ss.pickRow}>
            <TouchableOpacity style={ss.pickBtn} onPress={() => handle('camera')} disabled={busy} activeOpacity={0.8}>
              {busy
                ? <ActivityIndicator size="small" color={Colors.periwinkle} />
                : <><MaterialCommunityIcons name="camera-outline" size={16} color={Colors.periwinkle} /><Text style={ss.pickTxt}>Camera</Text></>}
            </TouchableOpacity>
            <TouchableOpacity style={ss.pickBtn} onPress={() => handle('gallery')} disabled={busy} activeOpacity={0.8}>
              ...
              <Text style={ss.pickTxt}>Gallery</Text>
            </TouchableOpacity>
          </View>
        </>
      )}
...
const ActingDriverPreTripScreen = () => {
  ...
  const onUpload = async () => {
    if (!allDone) {
      Alert.alert('Photos Required', 'Please capture all 4 vehicle photos before starting.');
      return;
    }
    try {
      setUploading(true);
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          <Text style={styles.title}>Vehicle Photos — Before Trip</Text>
          <Text style={styles.subtitle}>Capture all 4 angles before you start</Text>
        </View>
      </View>

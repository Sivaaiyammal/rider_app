...
import { Text, View, Animated, TouchableOpacity, ScrollView, Linking, Dimensions, TouchableWithoutFeedback } from 'react-native';
import MaterialCommunityIcons from 'react-native-vector-icons/MaterialCommunityIcons';
import Ionicons from 'react-native-vector-icons/Ionicons';
import DeviceInfo from 'react-native-device-info';

import Logo from "../../common/assets/icons/CompanyLogo.svg"
import DriverLogo from "../../notdriver/assets/icons/driver_logo.svg"
...
const SideDrawerPublicRides = () => {
  ...
  const {selectedLanguage, trackingMode} = useUserStore()
  ...
  const hasAllPermissions =
    ...
    hasBackgroundLocationPermission && BGLocationTask.isRunning() && hasUsageStatsPermission;

  const getDrawerData = () => {
    if (trackingMode === 'vehicle_tracking') {
      return drawerData.filter(item => item.id !== 1 && item.id !== 2 && item.id !== 4 && item.id !== 7 && item.id !== 8&& item.id !== 10);
    }
    if (trackingMode === 'mobile_tracking') {
      return drawerData.filter(item => item.id !== 5 && item.id !== 6 && item.id !== 8  && item.id !== 9);
    }
    ...
    <>
      ...
      <Animated.View
        ...
        <View style={driverDrawerStyles.subContainer}>
          ...
          <TouchableOpacity style={driverDrawerStyles.madeByBtn} onPress={()=> handlePress()}>

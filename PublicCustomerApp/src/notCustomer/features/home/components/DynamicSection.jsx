...
const ActingDriverUpdatedImage = require('../../../../common/assets/images/acting_driver_updated.png');
const ActingDriverUpdatedImage2 = require('../../../../common/assets/images/image.png');
const ActingDriverPIC = require('../../../../common/assets/images/actingDriverPic.png');
/* ------------------------------------------------------------------
...
const getServiceImage = serviceKey =>
  SERVICE_IMAGES[serviceKey] || SERVICE_IMAGES.default;
...
const DEFAULT_HORIZONTAL_BANNERS = [
  {
    ...
    bannerURI:
      'https://images.pexels.com/photos/210231/pexels-photo-210231.jpeg',
  },
  {
    id: 'mock-banner-2',
    title: 'Refer & Earn',
    ...
    bannerURI:
      'https://images.pexels.com/photos/135061/pexels-photo-135061.jpeg',
  },
  {
    id: 'mock-banner-3',
...
const getLocalizedValue = (item, field, language) => {
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(item, localizedKey) &&
      item[localizedKey]
    ) {
...
const getHorizontalTextContainerAlignment = (
  placement = DEFAULT_HORIZONTAL_TEXT_PLACEMENT,
) => {
...
const getHorizontalTextAlignment = (
  placement = DEFAULT_HORIZONTAL_TEXT_PLACEMENT,
) => {
  if (!placement || typeof placement !== 'string') {
    return null;
...
const DynamicSection = ({title = 'Dynamic Section', onSelect = () => {}}) => {
  ...
  const rawSectionConfig = Array.isArray(appConfig?.HOME_SCREEN_CONFIG)
    ...
    : [];
  const shouldInjectHorizontal = !rawSectionConfig.some(
    section => section?.key === 'horizontal_banners',
  );
  const SECTION_CONFIG = useMemo(
    () => (shouldInjectHorizontal ? [...rawSectionConfig] : rawSectionConfig),
  ...
  useEffect(() => {
    if (
      !Array.isArray(ENHANCED_SECTION_CONFIG) ||
      !ENHANCED_SECTION_CONFIG.length
    ) {
      return;
    }
    ...
    ENHANCED_SECTION_CONFIG.forEach(section => {
      ...
      if (Array.isArray(section?.resolvedItems)) {
        section.resolvedItems.forEach(itemWrapper => {
          if (
            itemWrapper?.imageSource &&
            typeof itemWrapper.imageSource.uri === 'string'
          ) {
  ...
  const renderHorizontalBannerList = section => {
    const isHorizontal =
      section?.isHorizontal !== undefined ? section.isHorizontal : true;
    ...
    const effectiveData =
      Array.isArray(section?.resolvedItems) && section.resolvedItems.length
        ? section.resolvedItems
        : [];

    const renderItem = ({item}) => {
      const bannerData = item?.original || item;
      const distanceLabel = formatKm(
        bannerData?.distanceKm ?? bannerData?.distance,
      );
      ...
      const textContainerAlignment =
        getHorizontalTextContainerAlignment(textPlacement);
      ...
      const titleValue =
        typeof (localizedTitle || bannerData?.name) === 'string'
          ? localizedTitle || bannerData.name
          : '';
      const isShortTitle = titleValue.length > 0 && titleValue.length <= 30;
      const cardWidth = Number.isFinite(section?.itemWidth)
        ? section.itemWidth
        : 280;
      ...
      return (
        <TouchableOpacity
          style={[styles.horizontalCard, {width: section?.itemWidth || 280}]}
          activeOpacity={0.85}
          ...
          distanceLabel ? (
            <LinearGradient
              ...
              <View
                style={[
                  styles.horizontalCardTextContainer,
                ...
                {localizedDescription || bannerData?.area ? (
                  <AdaptiveText
                    style={[
                      styles.horizontalCardDescription,
                      horizontalTextAlignment,
                    ]}>
                    {localizedDescription || bannerData.area}
                  </AdaptiveText>
                ...
                {localizedButtonText ? (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.85}
                    onPress={() =>
                      handleHorizontalBannerButtonPress(section, bannerData)
                    }>
                    <AdaptiveText style={styles.horizontalCardButtonText}>
                      {localizedButtonText}
                    </AdaptiveText>
                  </TouchableOpacity>
                ) : null}
              ...
              {distanceLabel ? (
                <View style={styles.horizontalCardDistanceChip}>
                  <AdaptiveText style={styles.horizontalCardDistanceText}>
                    {distanceLabel}
                  </AdaptiveText>
                </View>
              ) : null}
    ...
    const localizedSectionTitle =
      getLocalizedValue(section, 'sectionTitle', i18n.language) ||
    ...
    return (
      <View key={section.key} style={styles.horizontalSection}>
        ...
        (localizedSectionTitle || section.sectionTitle || section.title) ? (
            ...
            isHorizontal
              ...
              : {
                  paddingRight: Math.max(
                    contentPaddingHorizontal - gridSpacing,
                    0,
                  ),
                }
          }
      ...
      {(() => {
        ...
        return section.showSectionTitle !== false ? (
          <Text style={styles.title}>
            {localizedSectionTitle || section.sectionTitle || title}
          </Text>
        ) : null;
      })()}
      <FlatList
        ...
        renderItem={({item}) => (
          <TouchableOpacity
            ...
            <View
              ...
              <Image
                source={getServiceImage(item.key)}
                style={styles.image}
                resizeMode="contain"
              />
            </View>
            ...
            <Text
              style={[styles.label, {color: item.textColor || '#353535ff'}]}>
              {item.label}
            </Text>
          </TouchableOpacity>
        )}
    ...
    section?.bannerURI ? (
      <View key={section.key} style={styles.bannerSection}>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.85}
          onPress={() => handleBannerPress(section)}>
          <Image
            source={
              createRemoteSource(section.bannerURI) || SERVICE_IMAGES.default
            }
            style={[
              styles.bannerImage,
              {
            ...
            <View
              ...
              {section.text.title ? (
                <Text
                  style={[
                    styles.bannerTitle,
                    getTextAlignmentStyle(section.text.placement),
                  ]}>
                  {section.text.title}
                </Text>
              ) : null}

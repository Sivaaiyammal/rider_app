...
import {profileUpdateMutations} from '../../API/APICalls/UserAPICalls';
...
const RegisterationScreen = () => {
  ...
  const {mutate: RegisterMutate, isLoading: isLoading} =
    profileUpdateMutations(onRegisterSuccess);

  const onStepperBackHandler = () => {
    let max_stepper_length = FormStepper.length - 1;

    if (FormStepperState != 0 && FormStepperState <= max_stepper_length)
  ...
  const onStepperNextHandler = async id => {
    ...
    if (id == 'email' && !verifyEmail(Email))
      return alert(`Please enter valid email address.`);

    if (FormStepperState < max_stepper_length)
      setFormStepperState(FormStepperState + 1);
      ...
      <View style={{gap: 5}}>
        ...
        <View style={registerationStyles.stepperInputContianer}>
          <TextInput
            ...
            onFocus={value =>
              onStepperInputFocusHandler(
                FormStepper[FormStepperState].id,
                value,
              )
            }
            color="#000"
